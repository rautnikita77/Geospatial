...
if __name__ == "__main__":
    # Get coordinates for northwestern campus / Navy Pier
    (x1, y1), (x2, y2) = gps_to_ecef_pyproj([min_lat, min_lon]), gps_to_ecef_pyproj([max_lat, max_lon])
    x_min = min(x1, x2)
    y_min = min(y1, y2)
    x_max = max(x1, x2)
    y_max = max(y1, y2)
    nodes = get_nodes(min_lat, min_lon, max_lat, max_lon)
